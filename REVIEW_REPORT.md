...
- `v0.4.0` 增加口述资料库：录音/文字笔记、摘要、任意组合、本机 Agent 讨论和转选题入口。
- `v0.4.0` 增加本地文件夹素材索引：素材库可登记本地图片/视频/音频路径，不强制上传大文件，并支持本地音视频 Range 预览。
- `v0.4.1` 增加默认浅色主题和侧边栏深浅切换，提词器也跟随主题，改善长时间操作可读性。
- 录制页新增全屏提词器，支持速度、字号和键盘控制。
- 发布台新增 Agent 封面方案生成，输出悬念型、数字型、对比型 3 个可执行 brief。
- Modal 和脚本高亮补充 `role` / `aria-modal` / `aria-label`，无障碍基础更稳。
- Lint 已清零，新增 `typecheck` 和 `lint:fix` 脚本，单元测试提升到 25 个。
...
- **可读性**：默认浅色模式更适合白天和长时间使用，深色模式保留给弱光环境。
- **开题上下文**：创建选题时先固定“这条视频给谁什么承诺”，目标平台同步写到文件系统 truth。
- **录制执行**：提词器支持暂停、跳段和键盘控制，Take 管理会写回每段可用/备用/重录状态。
- **口述捕获**：零散录音可以先进入资料库，再组合成主题讨论和正式选题。
- **本地素材管理**：素材库可以索引本地文件夹，减少大文件上传和重复搬运。
...
| 等级 | 位置 | 问题 | 建议修复 |
|------|------|------|----------|
| **LOW** | 全局 | 原本没有测试覆盖，重构时缺乏信心。 | 已补 Vitest 基础覆盖；当前 25 个单元测试，并加入 typecheck。 |
...
| 测试 | 10.0 | Vitest 覆盖 validation、schema、pipeline、auth、pagination、DB path、recording takes、voice inbox、local folder import；当前 6 个测试文件 25 个测试 |
